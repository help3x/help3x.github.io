...
- 英語
	...
	- [熟語⇔略語集](http://www.twj.co.jp/helpline/shortword/main.htm)
	- [「英文快読術」に載っている英語勉強法を、アフィリンクを貼りながらまとめる](http://aodrey.hatenablog.com/entry/2016/12/06/170104)
...
- Oracle
	...
	- [OSコマンドによる物理バックアップの全手順](http://www.atmarkit.co.jp/ait/articles/0606/30/news130.html)
...
```
.gitignoreで除外設定したフォルダが除外にならない理由

`.gitignore`ファイルの文字コードがShift_JISになっていたからだった。  
文字コードをUTF-8（BOMなし）にして保存したら除外されるようになった。
```
...
<<   作成日時 ： 2008/03/31 16:17   >> 

ブログ気持玉 0 / トラックバック 5 / コメント 5
  
 いつもウェブリブログをご利用いただきありがとうございます。

 本日よりウェブリブログの編集用APIを公開いたします。

このたびのAPI公開に伴い、ブログエディタ「Windows Live(TM)
 Writer」での動作確認をしています。普及の進む同エディタへ
 の対応により、より一層、手軽でわかりやすい形でブログの
編集が可能になります。
...
です。
 ...
 　□使い方
 　　　1.ブログ編集用APIに対応したクライアントソフト *1 を使って
　　　　ブログの記事作成や編集をする。(Windows Live(TM) Writer
　　　　などご利用できます。）

 　　　2.ブログ編集用APIを使って、ツールを自作する。


 　　注意）*1. クライアント側のAPIによっては利用できない場合
 　　　　　　　　があります。動作を確認しているのは
　　　　　　　　Windows Live(TM) Writerのみです。
 　

 　□API呼び出しのエンドポイント
　　https://bblog.sso.biglobe.ne.jp/ap/xmlrpc


　□対応API一覧
 　　metaWeblog.newPost : 新規記事を作成する。
 　　metaWeblog.editPost : 既存記事を編集する。
 　　metaWeblog.getPost : 既存の特定の記事情報を取得する。
 　　metaWeblog.getCategories : 記事のテーマの一覧を取得する。
 　　metaWeblog.getRecentPosts : 既存の最新の記事情報を
　　　　　　　　　　　　　　　　　　　　　取得する。
 　　metWeblog.newMediaObject : ファイルをアップロードする。
 　　blogger.deletePost : 記事を削除する。
 　　blogger.getUsersBlogs : ブログ名とブログのURLを取得する。
 　　blogger.getUserInfo : ニックネームとブログのURLを取得する。


 尚、APIに関するお問い合わせはサポートいたしません。
 使い方についてはウェブリ広場で情報交換をお願いします。 





http://info.at.webry.info/200803/article_17.html
```

```
http://dc.watch.impress.co.jp/docs/news/dcm/1069276.html
...
[エアコンをつけたまま寝ると朝だるい理由　熱帯夜対策を再考する](https://news.yahoo.co.jp/byline/nishidamasaki/20170709-00073010/)
-------------

- 寝付きのエアコンは付ける。
- 時間としておよそ3時間 ＝ コア睡眠（ノンレム／レム睡眠が1～2サイクル含まれる時間）の分 ＝ は冷房を付けておく。
- 明け方（目覚める前）の冷え過ぎが、朝のだるさを生む。
- やや高めの温度設定（26～28℃ぐらい）にする。
- エアコンのオン・オフの繰り返しは身体的な負担が増える。
- 送風の強さを下げる ＝ 除湿（ドライ）の方がいいかも。
- 寝巻きは長袖長ズボン。

除湿にして温度は標準+1～2℃とする。  
冷房時間は、、、  
最低、20時30分～23時30分。  
寝付くまで1時間はかかると仮定し、0時30分までは付けることにする。

20時30分～翌2時30分？　これだと6時間。ここらを目安として運用してみる。
...
推奨サイズ
w 330mm x d 190mm x h 210mm


飼育容器
プラケース（中）
W300 x D200 x H205mm
...
| Size     | W   | D   | H   |目安      |
|:---------|----:|----:|----:|:---------|
| タイニー | 105 | 105 |  93 |小さなサイズのクワガタ・カブトに。|
| ミニ     | 180 | 110 | 145 |クワガタ♀・クワガタ♂60mm以下に最適です。|
| 小       | 230 | 153 | 173 |クワガタ♀・クワガタ♂60mm以上に最適です。|
| 中       | 298 | 195 | 210 |カブトムシ♂♀に最適です。 |
| 大       | 370 | 221 | 240 |カブトムシ♂♀に最適です。<br>ヘラクレス等大型カブトの幼虫飼育／クワガタ・カブトの採卵にも最適です。|
...
| Size     | W   | D   | H   | 容量 |
|:---------|----:|----:|----:|:-----|
| ノーマル | 181 | 124 | 102 | 1.4L |
| ラージ   | 267 | 187 | 159 | 5.6L |



ほか

- [クリアースライダー](http://www.e-mushi.com/kurisuracase.html)
- [デジケース](https://www.shopping-charm.jp/Items.aspx?tid=02&catId=1864151000)
...
羽化直後（蛹から変わった直後）は、羽を固めたり、体内の不要物を排出したりなど、活動に必要な体を蛹室（蛹の部屋）内で作っています。  
そのため、じっとしていることが多いと考えられています。  
そのままそっとしておき、2週間程度、様子を見てみましょう。

適切に温度管理されたカブトムシやクワガタは羽化しても1週間～2週間くらいは地上に出て来ません。  
これは外側は完成しているのですが、内蔵などの中身が固まりきれていないためです。

この時に無理やり掘り返して取り出すことは厳禁です。  
触った衝撃で内臓が破損し、死んでしまうこともありますので気長に自分から地上に出てくるのを待っていましょう。
...
羽化　7日～14日ぐらい
↓
成熟　羽化後にはじめてエサを食べてから1週間ぐらい。このときから多頭飼育、交尾が可能になる。
↓
成虫
↓
★
...
[IT業界のシュールな現実（3） &#x7c; 下流から見たIT業界](http://el.jibun.atmarkit.co.jp/karyu/2008/09/it-3-b6a8.html)
