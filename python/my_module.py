...
def get_prev_and_next_link(target, articles):
    prev_list = sorted(filter(lambda x: x.posted_at < target.posted_at, articles), \
                       key=operator.attrgetter("posted_at"), \
                       reverse=True)

    next_list = sorted(filter(lambda x: x.posted_at > target.posted_at, articles), \
                       key=operator.attrgetter("posted_at"))
    ...
    if prev_entry is not None:
        ...
        if entry_title is not None and len(entry_title) > 0:
            dict["prev-entry-link"] = prev_entry.url
            dict["prev-entry-name"] = entry_title

    if next_entry is not None:
        entry_title = get_pagetitle(next_entry.file_path)
        if entry_title is not None and len(entry_title) > 0:
            dict["next-entry-link"] = next_entry.url
            dict["next-entry-name"] = entry_title
...
def articles_getter(directory_path):
    ...
    flist = glob.glob(directory_path + "\\*.md")
    for path in flist:
